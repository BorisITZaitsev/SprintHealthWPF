...
namespace Statistic_functions
{
    ...
    public class DataTools
    {
        ...
        public static double standartstandardDeviationOfEvents;
        ...
        public static bool AnalyzeDistribution(Dictionary<DateTime, int[]> RWFstatistics)
        {
            ...
            additionalInformation += "Элементы: ";
            foreach (int element in elements) additionalInformation +=  element + " ";
            additionalInformation += "\n";

            int sum = elements.Sum();
            additionalInformation += "Summ: " + sum + "\n";
            double average = elements.Average();
            additionalInformation += "Среднее выборочное: " + " " + average + "\n";
            double variance = elements.Select(x => (x - average) * (x - average)).Average();
            additionalInformation += "Дисперсия: " + variance + "\n";
            double standardDeviation = Math.Sqrt(variance);
            additionalInformation += "Среднеквадратичное отклонение: " + standardDeviation + "\n";
            bool isNormalDistribution;
            int[] outstandingElements = Array.FindAll(elements, x => ((x > average + standardDeviation) || (x < average - standardDeviation)));
            additionalInformation += "Выдающиеся элементы: ";
            foreach (int element in outstandingElements) additionalInformation += element + " ";
            additionalInformation += "\n";
            double absolute = 0;
            foreach (int element in outstandingElements) absolute += Math.Abs(element - average);
            additionalInformation += "Сумма модулей разниц или Abs(эллемент - среднее выборочное): " + absolute + "\n";
            additionalInformation += "Сумма модулей разниц - (Станд. отклон * 1,3 * кол-во выдающихся элементов * 0,5:" + "\n";
            additionalInformation += $"{absolute} - (({standardDeviation * 3}) * {outstandingElements.Length} * 0.5) = {absolute} - {standardDeviation * 3 * outstandingElements.Length * 0.5} = " +
                $"{absolute - ((standardDeviation * 3) * outstandingElements.Length * 0.5)}";
            if ((absolute - ((standardDeviation * 3) * outstandingElements.Length * 0.5)) > 0) isNormalDistribution = true;
            else isNormalDistribution = false;

            avarageOfEvents = average;
            standartstandardDeviationOfEvents = standardDeviation;
